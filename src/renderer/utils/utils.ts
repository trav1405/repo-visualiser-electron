...
export const keepBetween = (min: number, max: number, value: number) => Math.max(min, Math.min(max, value));

export const getPositionFromAngleAndDistance = (angle: number, distance: number): [number, number] => {
  const radians = (angle / 180) * Math.PI;
  return [Math.cos(radians) * distance, Math.sin(radians) * distance];
};

export const getAngleFromPosition = (x: number, y: number): number => (Math.atan2(y, x) * 180) / Math.PI;
